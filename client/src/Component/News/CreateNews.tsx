...
const CreateNews: React.FC = () => {
    ...
    async function addNew() {
        await axios
            .get(`https://newsapi.org/v2/top-headlines?q=${search}&from=${NowDate}&apiKey=${API_KEY_NEWS}`)
            .then((response) => {
    ...
    return (
        <div>
            <input
                ...
                className="inpnews"
            />
            <button className="btnnews" onClick={clickHandler} type="button">
                Поиск
            </button>
            <Routes>
                <Route path="/" element={<NewsBox news={news} />} />
                {news.map(({}, idx) => (
                    <Route path={`/${idx}`} element={<NewsContent news={news} idx={idx} />} />
                ))}
            </Routes>
        </div>
    );
